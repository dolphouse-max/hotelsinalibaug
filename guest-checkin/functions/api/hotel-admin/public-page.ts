    ...
    await context.env.DB.prepare(
      `UPDATE hotel_public_pages
       SET public_title = ?2,
           ...
           check_in_time = ?18,
           check_out_time = ?19,
           room_count_display = ?20,
           distance_from_beach = ?21,
           distance_from_local_bus_stop = ?22,
           distance_from_alibaug_bus_stand = ?23,
           distance_from_mandwa_jetty = ?24,
           beach_distance_meters = ?25,
           beach_distance_label = ?26,
           room_types_json = ?27,
           amenities_json = ?28,
           faq_json = ?29,
           nearby_places_json = ?30,
           policies_json = ?31,
           inquiry_whatsapp_prefill = ?32,
           updated_at = CURRENT_TIMESTAMP
       WHERE id = ?1`
    )
      .bind(
