  ...
  return normalizePublicPagePayload(
    {
      ...
      contact_person_name: normalizeText(formData.get("contact_person_name")),
      address_line_1: normalizeText(formData.get("address_line_1")),
      address_village: normalizeText(formData.get("address_village")),
      address_taluka: normalizeText(formData.get("address_taluka")),
      address_district: normalizeText(formData.get("address_district")),
      address_pincode: normalizeText(formData.get("address_pincode")),
      ...
      latitude: normalizeText(formData.get("latitude")),
      longitude: normalizeText(formData.get("longitude")),
    ...
    await context.env.DB.prepare(
      `UPDATE hotel_public_pages
       SET public_title = ?2,
           ...
           inquiry_email = ?18,
           website_url = ?19,
           google_maps_embed_url = ?20,
           google_maps_place_url = ?21,
           latitude = ?22,
           longitude = ?23,
           check_in_time = ?24,
           check_out_time = ?25,
           room_count_display = ?26,
           distance_from_beach = ?27,
           distance_from_local_bus_stop = ?28,
           distance_from_alibaug_bus_stand = ?29,
           distance_from_mandwa_jetty = ?30,
           beach_distance_meters = ?31,
           beach_distance_label = ?32,
           room_types_json = ?33,
           amenities_json = ?34,
           faq_json = ?35,
           nearby_places_json = ?36,
           policies_json = ?37,
           inquiry_whatsapp_prefill = ?38,
           updated_at = CURRENT_TIMESTAMP
       WHERE id = ?1`
    )
      .bind(
        ...
        payload.addressVillage,
        payload.addressTaluka,
