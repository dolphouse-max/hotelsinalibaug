    ...
    if (existingByHotel) {
      const result = await context.env.DB.prepare(
        `UPDATE hotel_public_pages
         SET category = ?2,
             ...
             inquiry_email = ?20,
             website_url = ?21,
             google_maps_embed_url = ?22,
             google_maps_place_url = ?23,
             latitude = ?24,
             longitude = ?25,
             check_in_time = ?26,
             check_out_time = ?27,
             room_count_display = ?28,
             distance_from_beach = ?29,
             distance_from_local_bus_stop = ?30,
             distance_from_alibaug_bus_stand = ?31,
             distance_from_mandwa_jetty = ?32,
             beach_distance_meters = ?33,
             beach_distance_label = ?34,
             room_types_json = ?35,
             amenities_json = ?36,
             faq_json = ?37,
             nearby_places_json = ?38,
             policies_json = ?39,
             inquiry_whatsapp_prefill = ?40,
             canonical_path = ?41,
             is_published = ?42,
             sort_order = ?43,
             last_reviewed_by = ?44,
        ...
        .bind(
          ...
          normalized.distanceFromBeach,
          normalized.distanceFromLocalBusStop,
          normalized.distanceFromAlibaugBusStand,
          normalized.distanceFromMandwaJetty,
      ...
      const result = await context.env.DB.prepare(
        `INSERT INTO hotel_public_pages (
           ...
           room_count_display,
           distance_from_beach,
           distance_from_local_bus_stop,
           distance_from_alibaug_bus_stand,
           distance_from_mandwa_jetty,
